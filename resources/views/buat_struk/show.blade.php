    ...
    <div class="bg-white shadow-2xl md:rounded-3xl overflow-hidden border border-gray-100 print:shadow-none print:border-none print:m-0">
        ...
        <div class="p-6 md:p-12 print:p-4 print:pt-0">
            ...
            <div class="header-section flex justify-between items-start print:mb-4">
                <div class="flex-1">
                    <p class="text-xl font-bold text-gray-800 print:text-lg">{{ $toko->nama_toko ?? 'Nama Toko Tidak Diketahui' }}</p>
                    <p class="text-sm text-gray-500 max-w-xs print:text-[11px] print:leading-tight">{{ $toko->alamat_toko ?? 'Alamat Toko Tidak Diketahui' }}</p>
                    <p class="text-sm text-gray-600 font-medium pt-1 print:text-[11px]">WA: {{ $toko->no_telepon ?? 'No. Telepon Tidak Diketahui' }}</p>
                </div>
            ...
            <div class="flex justify-between items-start print:flex print:flex-row">
                <div class="flex-1 space-y-0.5">
                    ...
                    <p class="text-xs font-bold text-gray-700 print:text-[10px]">{{ $toko->nama_bank ?? 'Nama Bank Tidak Diketahui' }} - {{ $toko->no_rekening ?? 'No. Rekening Tidak Diketahui' }}</p>
                    <p class="text-[10px] text-gray-500 uppercase">A/N {{ $toko->pemilik_rekening ?? 'Pemilik Rekening Tidak Diketahui' }}</p>
                </div>
            ...
            <div class="mt-8 flex justify-end text-center print:mt-6">
                <div class="w-40">
                    ...
                    <p class="font-black text-sm text-gray-800 border-b border-gray-200 pb-1 uppercase">{{ $toko->nama_toko ?? 'Nama Toko Tidak Diketahui' }}</p>
                </div>
            </div>
        </div>
    </div>
